...
public:
    ...
    display_battle(uint x = 1600, uint y = 1000, std::string game_name = "Giereczka", uint max_hand_size = 8);  //konstruktor domyślno-parametryczny
    ~display_battle(); //destruktor

    //metody klasy:
    void run_battle();  //wywołuje ekran bitwy
    void new_frame() override;  //rysuje nową klatkę na ekran
};
